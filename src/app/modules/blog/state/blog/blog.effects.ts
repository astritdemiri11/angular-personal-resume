...
export class BlogEffects {
  loadBlogs$ = createEffect(() => this.actions$.pipe(
    ...
    map(response => {
      const items = this.blogService.business.convertDTOs(response.data);
      return BlogActions.loadBlogsSuccess({ languageCode: response.append.languageCode, items });
    })
  ));

  loadBlogContent$ = createEffect(() => this.actions$.pipe(
    ofType(BlogActions.loadBlogContent),
    switchMap(action => {
      return this.blogService.request.loadBlogContent(action.path, action.name, action.languageCode);
    }),
    catchError((error: CustomError<{ name: string, languageCode: LanguageCode }>, caught) => {
      this.store.dispatch(BlogActions.loadBlogContentFailure({ name: error.append.name, languageCode: error.append.languageCode, error: error.message }));
      return caught;
    }),
    map(response => {
      return BlogActions.loadBlogContentSuccess({ name: response.append.name, languageCode: response.append.languageCode, content: response.data });
    })
  ));
